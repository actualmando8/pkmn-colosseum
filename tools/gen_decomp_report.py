...
def _measures(total_funcs, matched_funcs, total_code, matched_code, fuzzy,
              total_units, complete_units) -> dict:
    ...
    100%. Data fields are 0 (we measure .text / code only) and are omitted.
    """
    m = {}
    ...
    # total_data / matched_data / matched_data_percent omitted (all 0)
    ...
    # complete_data / complete_data_percent omitted (0)
    if total_units:
...
def validate(report, agg):
    ...
    needed = {"total_functions", "matched_functions", "total_code",
              "matched_code", "fuzzy_match_percent", "total_units"}
    ...
    lines.append("encoding: total_code is str, total_functions is int, "
...
def main() -> int:
    ...
    m = report["measures"]
    print("overall: %s/%s functions (%.2f%%), %s/%s code bytes, fuzzy %.4f%%"

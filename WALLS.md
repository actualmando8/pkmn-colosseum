...
| `fn_801A40F8` | hsd_lobj.c | structural — `HSD_ClassInfoHead` in include/hsd/hsd_class.h is one pointer-slot too large → every method dispatch off by +4 (release@0x30/destroy@0x34 vs ours @0x34/@0x38); also `lbl_8036CA20` vs `&hsdLObj` dispatch base | 93.28% | no (C active) | NOT a true wall — header fix candidate, but touching hsd_class.h re-bases every HSD TU; needs a dedicated cross-TU pass. |

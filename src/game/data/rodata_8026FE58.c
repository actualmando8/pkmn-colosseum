#include "dolphin/types.h"

/* Auto-carved .rodata unit 0x8026FE2C..0x8026FFE0 (10 objects). Non-relocated data emitted byte-exact as const u8[]; pointer/jump tables as const void*[] for R_PPC_ADDR32 relocations. */

const u8 lbl_8026FE2C[19] = {
    0x43, 0x41, 0x4C, 0x4C, 0x49, 0x4E, 0x47, 0x20, 0x45, 0x58, 0x49, 0x32, 0x5F, 0x49, 0x6E, 0x69,
    0x74, 0x0A, 0x00,
};

const u8 gap_04_8026FE3F_rodata[1] = {
    0x00,
};

const u8 lbl_8026FE40[24] = {
    0x44, 0x4F, 0x4E, 0x45, 0x20, 0x43, 0x41, 0x4C, 0x4C, 0x49, 0x4E, 0x47, 0x20, 0x45, 0x58, 0x49,
    0x32, 0x5F, 0x49, 0x6E, 0x69, 0x74, 0x0A, 0x00,
};

const u8 lbl_8026FE58[24] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x41, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x41, 0xE0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

const u8 lbl_8026FE70[54] = {
    0x47, 0x43, 0x4E, 0x5F, 0x4D, 0x65, 0x6D, 0x5F, 0x41, 0x6C, 0x6C, 0x6F, 0x63, 0x2E, 0x63, 0x20,
    0x3A, 0x20, 0x49, 0x6E, 0x69, 0x74, 0x44, 0x65, 0x66, 0x61, 0x75, 0x6C, 0x74, 0x48, 0x65, 0x61,
    0x70, 0x2E, 0x20, 0x4E, 0x6F, 0x20, 0x48, 0x65, 0x61, 0x70, 0x20, 0x41, 0x76, 0x61, 0x69, 0x6C,
    0x61, 0x62, 0x6C, 0x65, 0x0A, 0x00,
};

const u8 gap_04_8026FEA6_rodata[2] = {
    0x00, 0x00,
};

const u8 lbl_8026FEA8[57] = {
    0x4D, 0x65, 0x74, 0x72, 0x6F, 0x77, 0x65, 0x72, 0x6B, 0x73, 0x20, 0x43, 0x57, 0x20, 0x72, 0x75,
    0x6E, 0x74, 0x69, 0x6D, 0x65, 0x20, 0x6C, 0x69, 0x62, 0x72, 0x61, 0x72, 0x79, 0x20, 0x69, 0x6E,
    0x69, 0x74, 0x69, 0x61, 0x6C, 0x69, 0x7A, 0x69, 0x6E, 0x67, 0x20, 0x64, 0x65, 0x66, 0x61, 0x75,
    0x6C, 0x74, 0x20, 0x68, 0x65, 0x61, 0x70, 0x0A, 0x00,
};

const u8 gap_04_8026FEE1_rodata[7] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

const u8 lbl_8026FEE8[24] = {
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x24,
    0x00, 0x00, 0x00, 0x34, 0x00, 0x00, 0x00, 0x44,
};

const u8 lbl_8026FF00[224] = {
    ,
};

 ...
 * Address range: 0x800DFEEC - 0x800E0678 (37 functions)
 *
 * ~37 tiny vector methods (0x10-0x44) incl. fn_800DFEEC(0xAC~GSvecTransformQuat 0x8C). One real TU: Colosseum's GSvec.cpp with all methods out-of-line; XD moved them to header inlines so the linker's surviving weak copies live in floorFieldCamera.cpp/floor.cpp (0x8012481C/4C/7C, 0x80121004) - hence the 4 anchors are non-monotonic dedup artifacts, NOT ordering evidence. Strict gate-2 fails until these 4 anchors are demoted/exempted as weak copies; after demotion the segment passes trivially....
 *

...
def scrub(source: str, target: str, force_target: bool) -> tuple[str, int, bool]:
    ...
    while i < n:
        ...
        # Keep every function whose body is real C (siblings included — MWCC
        # `-inline auto` may inline same-TU callees into the target, so their
        # bodies MUST survive for the isolated compile to stay faithful).
        # Reduce only genuinely asm-bodied definitions to forward decls, plus
        # the target itself under --force-target (diagnostics only).
        keep_body = not body_is_asm(body) and not (is_target and force_target)
        if keep_body:

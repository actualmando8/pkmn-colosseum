...
export WSLENV='MODELS:AGENT_NAME:TIER_FILTER:MAX_FN_ASM_LINES:MIN_PRIOR_PCT:REPAIR_ROUNDS:PLATEAU_BREAK_AFTER:FILE_FILTER'
echo "[deepseek] starting (model=$MODELS tiers=$TIER_FILTER cap=$MAX_FN_ASM_LINES min_prior=$MIN_PRIOR_PCT) ..."
while true; do

/**
 * @file gs_math_range_800E09E8.c
 * @brief GS render engine segment -- split from gs_render.c.
 *
 * XD source unit: unknown (random/noise math utilities; _fadeEffectGetRandom is a byte-match to a static in XD fade_effect.cpp)
 * Address range: 0x800E09E8 - 0x800E0D24 (7 functions)
 *
 * 7 fns (0x1B8,0x44,0x20,0x50,0x24,0x28,0x84) between GSlerp and GSutil. The lone anchor _fadeEffectGetRandom(0x50) is a generic LCG helper byte-matched to a LOCAL static deep inside XD's huge fade_effect.cpp (0x801A8164-0x801B715C) - almost certainly an identical-code collision, not TU evidence. Likely a Colosseum GSmath random/utility TU absorbed elsewhere in XD. Single anchor -> trivially monotonic.
 *

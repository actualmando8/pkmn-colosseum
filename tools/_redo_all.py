#!/usr/bin/env python3
"""Full redo of all goto conversions, preserving CRLF."""
import re, os, sys
...
from convert_gotos_safe import (
    ...
    pass_goto_in_if_else
)

excluded_base = ['gs_field_world.c', 'menu_common_ext.c', 'menu_tool.c', 'ui_core.c',
            'battle_logic.c', 'gba_conv.c', 'menu_middle.c', 'colosseum_script.c',
            'colosseum_event.c', 'colosseum_battle.c', 'gs_render.c', 'gs_model.c', 'late_game.c',
            'menu_precine.c']
...
passes_safe = [
    ...
    ('goto_in_if_else', pass_goto_in_if_else),
...
for root, dirs, files in os.walk('src'):
    for fn in files:
        if not fn.endswith('.c') or any(fn == ex for ex in excluded_base):
            continue
        ...
        # Phase 1: All safe passes + goto_in_if_else
        prev = initial
        ...
        if not ok:
            print('  REVERT %s: brace imbalance at %d' % (path, bad + 1))
            lines = original
            continue
...
print('\nTotal gotos removed: %d' % total_removed)

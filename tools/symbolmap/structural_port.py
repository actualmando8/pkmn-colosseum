...
def fingerprints(asm_dir: Path) -> dict:
    ...
    PORTING NOTE (current tree, 2026-07): the live asm tree is organized
    per-unit under subdirectories, not flat `auto_*text*.s` files (see
    mine_xrefs.py's porting note). Walk every `*.s` file recursively
    instead; `.fn` blocks only ever occur inside .text/.init sections so no
    section filter is needed here (unlike mine_xrefs.py, this pass doesn't
    need to disambiguate strings from code).
    """
    out = {}

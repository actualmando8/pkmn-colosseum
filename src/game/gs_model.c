/**
 * @file gs_model.c
 * @brief menu -- window/menu UI subsystem (kanji-era "menu" family).
 *
 * The real unit for this translation unit spans 0x801019F8 - 0x80109C88
 * (per config/GC6E01/splits.txt) and contains the `menu*`/`win*` window
 * and cursor management functions (menuOpen, menuSetDisp, winSetSequence,
 * winSpriteAdd, etc.), not a "GSmodel" 3D model manager.
 *
 * A prior transplant pass had introduced ten fictional `GSmodel_*`
 * functions (GSmodel_FindLoadedResource, GSmodel_ClearResourceTable,
 * GSmodel_AttachAnimation, GSmodel_UpdateAnimation, GSmodel_RenderModel,
 * GSmodel_BuildDisplayList, GSmodel_ProcessSkinning, GSmodel_GetResourceCount,
 * GSmodel_GetResourceByIndex, GSmodel_GetJointCount) with invented bodies
 * and addresses (some claiming 0x80101910, which is outside this unit's
 * actual range). None of these names appear in config/GC6E01/symbols.txt,
 * none were referenced anywhere except this file's own header, and one
 * (GSmodel_ClearResourceTable) duplicated the real, already-matched
 * fn_801019F8. They have been removed; see include/game/gs_model.h.
 */

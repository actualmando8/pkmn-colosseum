...
Reads dtk's symbol-resolved disassembly (build/GC6E01/asm/**/*.s) and
...
PORTING NOTE (current tree, 2026-07): the previous campaign's asm tree was
flat and every dtk-emitted file used the `auto_NN_ADDR_section.s` naming
convention, so the original miner just globbed `auto_*.s` at the top level
of --asm-dir and read the section kind out of the filename suffix.

The live tree's asm is organized per translation-unit under subdirectories
(build/GC6E01/asm/game/**, .../hsd/**, .../dolphin/**, ...) and most files
are named after their source unit (e.g. `hsd_sdata2_8047DD90.s`,
`gs_render.s`), not `auto_*`. Only ~150 residual `auto_NN_ADDR_text.s`
files remain (large spans of still-unattributed GAP functions - the exact
population this pipeline exists to help name) plus a handful of
`auto_NN_ADDR_sdata2.s` / `_rodata.s` data blobs.

So the only change made here is *plumbing*: instead of matching filenames,
we walk **every** `*.s` file under --asm-dir recursively and track the
active section from the assembler directives themselves - either the bare
form dtk emits for code/rodata (`.text`, `.rodata`, ...) or the `.section
.NAME, "a"` form it emits for sdata/sdata2/bss-like sections. The
`.fn`/`.endfn` and `.obj`/`.endobj` block-parsing logic below (the actual
evidence-mining logic) is untouched from the archived version.
...
DATA_SECTIONS = ("rodata", "data", "sdata2", "sdata")
# Section kinds that can hold executable functions.
CODE_SECTIONS = ("text", "init")

# dtk emits section changes two ways depending on how the object was
# split: a bare directive (`.text`, `.rodata`, ...) for auto-split/code
# objects, or an explicit `.section .NAME, "a"` for named sdata/sdata2/bss
# objects. Track whichever one we see so string/function extraction no
# longer depends on the filename.
_KINDS = r"(text|rodata|data|sdata2|sdata|bss|sbss|sbss2|init|ctors|dtors)"
SECTION_BARE = re.compile(rf"^\.{_KINDS}\b")
SECTION_DIRECTIVE = re.compile(rf"^\.section\s+\.{_KINDS}\b")


def _section_of(line: str, current: str | None) -> str | None:
    m = SECTION_DIRECTIVE.match(line) or SECTION_BARE.match(line)
...
def parse_strings(asm_dir: Path) -> dict:
    ...
    for s in sorted(asm_dir.rglob("*.s")):
        ...
        while i < len(lines):
            ...
            if m_obj and section in DATA_SECTIONS:
...
def parse_functions(asm_dir: Path) -> dict:
    ...
    for s in sorted(asm_dir.rglob("*.s")):
        ...
        while i < len(lines):
            ...
            if m_fn and section in CODE_SECTIONS:
...
def main() -> None:
    ...
    print(f"[mine] scanning {asm_dir} (recursive) ...")
